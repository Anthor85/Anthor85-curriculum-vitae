...
export const useFormacionComplementariaStore = () => {
	...
	const { formacionComplementaria, loading, error } = useSelector((state: any) => state.formacionComplementaria);
	...
	const deleteFormacionComplementaria = async (id: string) => {
		try {
			...
			console.log('formacionComplementaria deleted:', formacionComplementaria, data);
				
			dispatch(
				setFormacionComplementaria(formacionComplementaria.filter((form: any) => form.id !== data.formacionComplementaria.id))
			);
		} catch (error) {
